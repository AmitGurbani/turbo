...
/// [Asset]: crate::asset::Asset
#[turbo_tasks::function]
pub async fn all_referenced_output_assets(asset: OutputAssetVc) -> Result<OutputAssetsVc> {
    let references_set = asset.references().await?;
    let mut assets = Vec::new();
    let mut queue = VecDeque::with_capacity(32);
    for reference in references_set.iter() {
    ...
    let output_assets = assets
        ...
        .flatten()
        .collect();
    Ok(OutputAssetsVc::cell(output_assets))
}
...
/// [Asset]: crate::asset::Asset
#[turbo_tasks::function]
pub async fn all_referenced_modules(module: ModuleVc) -> Result<ModulesVc> {
    ...
    let modules = assets
        .into_iter()
        .map(|asset| async move { Ok(ModuleVc::resolve_from(asset).await?) })
        .try_join()
        .await?
        .into_iter()
        .flatten()
        .collect();
    Ok(ModulesVc::cell(modules))
}
...
#[turbo_tasks::function]
pub async fn primary_referenced_modules(module: ModuleVc) -> Result<ModulesVc> {
    let modules = module
        .references()
        .await?
        .iter()
...
pub async fn primary_referenced_output_assets(
    output_asset: OutputAssetVc,
) -> Result<OutputAssetsVc> {
    let output_assets = output_asset
        .references()
        .await?
        .iter()
        .map(|reference| async {
...
#[turbo_tasks::function]
pub async fn all_modules(asset: ModuleVc) -> Result<ModulesVc> {
    ...
    Ok(ModulesVc::cell(assets.into_iter().collect()))
}
